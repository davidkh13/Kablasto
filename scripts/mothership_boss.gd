...
func _track_player(delta: float) -> void:
	...
	if angle_diff < deg_to_rad(10) and distance < 650.0:
		linear_velocity = Vector2.ZERO
...
func _do_charge(delta: float) -> void:
	...
	linear_velocity = Vector2.UP.rotated(rotation) * charge_speed
...
func _on_shoot_timer_timeout():
	if game_manager:
		if game_manager.lives < 0:
			shoot_timer.stop()
			return
	spawn_mothership($Marker2D.global_position)
	spawn_mothership($Marker2D2.global_position)
	
	
func spawn_mothership(p: Vector2) -> void:
	var spawn_pos = p
	_glow_effect_small(spawn_pos)  # Add glow first
	
	await get_tree().create_timer(0.2).timeout  # Short delay to sync with the glow
	
	var mothership = game_manager._get_mothership()
	mothership.global_position = spawn_pos
	
	if Globals.player:
		mothership.direction = (Globals.player.global_position - mothership.global_position).normalized()
	...
	if not mothership.mothership_destroyed.is_connected(game_manager._on_mothership_destroyed):
		mothership.mothership_destroyed.connect(game_manager._on_mothership_destroyed)
